...
function renderTrainingPlan(){
    ...
    const targetSports=weakSports.length?weakSports:Object.keys(r.scores).slice(0,2);
...
function showView(v){pauseLearningVideos(); if(v==='recommendation'||v==='profile')v='results';document.querySelectorAll('.view').forEach(el=>el.classList.add('hidden'));const target=document.getElementById('view-'+v);if(target){target.classList.remove('hidden');target.classList.add('fade-in');}window.scrollTo(0,0);if(v==='assessment')renderStudentToolbar();if(v==='results'&&assessmentResult)renderResults();if(v==='coach')setTimeout(()=>document.getElementById('chatInput')?.focus(),300);}
function navigateAfterData(){if(!assessmentResult){alert('请先完成测评。');showView('assessment');return;}showView('results');}
...
const originalRenderResults=renderResults;const _renderResults=renderResults;renderResults=function(){_renderResults();enhanceResultsDetails(assessmentResult);setTimeout(()=>fillQuickHistory(assessmentResult.record),0);};
...
const safeUiActions={showView,startDemoExperience,navigateAfterData,showLearningDetail,generatePractice,updateGenderFields,submitAssessment,saveStudentProfile,showStudentManager,showHistory,generateTrainingPlan,generateHomework,showFullReport,resetAssessment,switchPlan,switchStudent,newStudent,deleteAssessment,sendChat,selectCheckin,selectFeeling};
